...
// The flow's settings: the values a template asks for once and any number of
// steps then read as {{var.<key>}}. Two audiences share this panel — whoever
// runs the flow edits the values, whoever authors it defines the fields — so
// the author controls live behind a toggle rather than in a separate screen.
//
// A variable only does something once a STEP references it, which is the part
// nobody guesses. Hence the per-variable usage count: a variable no step reads
// is dead weight, and saying so is the difference between the panel teaching the
// mechanism and the panel silently doing nothing.
...
/** A half-typed key can hold anything, so it is never spliced into a regex raw. */
function escapeRegExp(value: string): string {
  return value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

// How many steps actually read the variable. Comments are documentation, not
// reads. Matches both the {{var.key}} interpolation form and the vars['var.key']
// form a js step uses to read the value without interpolating it into its source.
...
const UsageHint: React.FC<{ usages: number; t: (k: string) => string }> = ({ usages, t }) => {
  if (usages > 0) {
    return (
      <Text fz="xs" c="dimmed">
        {t('agentflow.variables.usedIn').replace('{{count}}', String(usages))}
      </Text>
    );
  }
  return (
    <Group gap={4} wrap="nowrap" align="center">
      ...
      <Text fz="xs" c="orange">{t('agentflow.variables.unused')}</Text>
    </Group>
  );
};
  ...
  >
    <Stack gap="xs">
      <Group gap="xs" wrap="nowrap" align="flex-start">
        <AppTextInput
          label={t('agentflow.variables.key')}
          value={variable.key}
        ...
        />
        <SelectDropdown
          label={t('agentflow.variables.type')}
          options={FLOW_VARIABLE_TYPES.map((type) => ({
            value: type,
            label: t(`agentflow.variables.type.${type}`),
          }))}
          value={variable.type}
          ...
          style={{ flex: 1 }}
        />
        <Tooltip label={t('agentflow.variables.remove')}>
      ...
      <Group gap="xs" wrap="nowrap" align="flex-start">
        <AppTextInput
          label={t('agentflow.variables.label')}
        ...
        />
        <AppTextInput
          label={t('agentflow.variables.question')}
          placeholder={t('agentflow.variables.question.placeholder')}
      ...
      <AppTextInput
        label={t('agentflow.variables.hint')}
        placeholder={t('agentflow.variables.hint.placeholder')}
        value={variable.hint ?? ''}
      ...
      {variable.type === 'select' && (
        <AppTextInput
          label={t('agentflow.variables.options')}
          placeholder={t('agentflow.variables.options.placeholder')}
      ...
      {/* The value lives here too, not only in the collapsed value view: creating
          a variable and then having nowhere to type its value is where the whole
          mechanism stalls. */}
      ...
      <Group justify="space-between" align="center">
        ...
        <Group gap="md" align="center">
          {(variable.type === 'text' || variable.type === 'url' || variable.type === 'feed') && (
            <ToggleSwitch
              label={t('agentflow.variables.multilineToggle')}
              size="sm"
          ...
          <ToggleSwitch
            label={t('agentflow.variables.requiredToggle')}
            size="sm"
...
export const FlowVariablesPanel: React.FC<FlowVariablesPanelProps> = ({ flow, t, onChange }) => {
  ...
  // A duplicate or malformed key silently shadows/loses a value at run time, so
  // it is surfaced on the field that caused it rather than at save time.
  const keyErrors = useMemo(() => {
    const seen = new Map<string, number>();
    return variables.map((v) => {
      if (!FLOW_VAR_KEY_RE.test(v.key)) return t('agentflow.variables.key.invalid');
      const count = (seen.get(v.key) ?? 0) + 1;
      seen.set(v.key, count);
      return count > 1 ? t('agentflow.variables.key.duplicate') : undefined;
    });
  }, [variables, t]);
        ...
        <Group justify="space-between" align="center">
          <Group gap="xs" align="center">
            <Text fw={600} fz="sm" c="var(--mantine-color-default-color)">
              {t('agentflow.variables')}
            </Text>
            {missingCount > 0 && (
              <Badge color="red" variant="light" size="sm">
                {t('agentflow.variables.missingBadge')}
              </Badge>
            )}
          </Group>
          <Group gap="xs">
            {variables.length > 0 && (
              ...
              >
                {editing ? t('agentflow.variables.doneEditing') : t('agentflow.variables.edit')}
              </Button>
            )}
            <Button variant="default" size="xs" leftSection={<Plus size={14} />} onClick={handleAdd}>
              {t('agentflow.variables.add')}
            </Button>
          </Group>
        </Group>

        {variables.length === 0 ? (
          <Text fz="xs" c="dimmed" style={{ lineHeight: 1.6 }}>{t('agentflow.variables.empty')}</Text>
        ) : (
          <>
            {/* Creating a variable is only half of it — nothing happens until a
                step reads it, and the step's own variable chips are where that
                happens. Say so, once, right here. */}
            <Text fz="xs" c="dimmed" style={{ lineHeight: 1.6 }}>
              {t('agentflow.variables.howto')}
            </Text>
