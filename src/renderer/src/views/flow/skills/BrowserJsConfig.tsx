...
const HELPER_ROWS: { sig: string; key: string }[] = [
  ...
  { sig: 'await fill(sel, value)', key: 'flow.skill.browser_js.help.fill' },
  { sig: 'click(sel)', key: 'flow.skill.browser_js.help.click' },
  { sig: 'read(sel)', key: 'flow.skill.browser_js.help.read' },
  { sig: 'readAll(sel)', key: 'flow.skill.browser_js.help.readAll' },
  { sig: 'await sleep(ms)', key: 'flow.skill.browser_js.help.sleep' },
  { sig: 'await screenshot(name?)', key: 'flow.skill.browser_js.help.screenshot' },
];

const BROWSER_JS_LLM_REFERENCE = [
  'You are writing the `code` for a Yobi flow "browser_js" step.',
...
export const BrowserJsConfig: React.FC<SkillConfigProps> = ({ step, onChange, t }) => {
  ...
  return (
    <Stack gap="xs">
      <AppTextInput
        label={t('flow.skill.browser_js.page')}
        placeholder={t('flow.skill.browser_js.page.placeholder')}
      ...
      <Box>
        <Group justify="space-between" align="center" mb={4} wrap="nowrap">
          <Text fz="sm" fw={500}>{t('flow.skill.browser_js.code')}</Text>
          <AppButton
            variant="subtle"
            size="xs"
          ...
          >
            {t('flow.skill.browser_js.helpers')}
          </AppButton>
        </Group>
      ...
      <Collapse expanded={helpOpen}>
        <Paper withBorder p="xs" radius="sm">
          <Stack gap={6}>
            <Text fz="xs" c="dimmed">{t('flow.skill.browser_js.help.selectors')}</Text>
            {HELPER_ROWS.map((row) => (
            ...
            <Text fz="xs" c="dimmed">{t('flow.skill.browser_js.help.return')}</Text>
            <AppButton
              variant="light"
              size="xs"
            ...
            >
              {copied ? t('flow.skill.browser_js.help.copied') : t('flow.skill.browser_js.help.copy')}
            </AppButton>
          </Stack>
        </Paper>
      </Collapse>
      ...
      <SettingRow
        ...
        label={t('flow.skill.browser_js.emitFailFlag')}
        hint={t('flow.skill.browser_js.emitFailFlag.hint')}
        control={
          <ToggleSwitch
      ...
      <Text fz="xs" c="dimmed">
        {t('flow.skill.browser_js.outputHint').split('{{outputKey}}').join(`{{${step.outputKey}}}`)}
      </Text>
    </Stack>
  );
};
