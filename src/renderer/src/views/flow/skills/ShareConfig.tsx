...
export const ShareConfig: React.FC<SkillConfigProps> = ({ step, onChange, t }) => {
  ...
  const supportedExpires = resolveShareExpires(
    settings?.instanceUrl ?? DEFAULT_SHARE_INSTANCE,
    settings?.instanceExpires,
  );
  const rawExpire = step.config.expire || settings?.expire || '1week';
  const storedExpire: ShareExpire = isShareExpire(rawExpire) ? rawExpire : (settings?.expire ?? '1week');
      ...
      {sharing ? (
        <>
          ...
          <SelectDropdown
            ...
            options={supportedExpires.map((value) => ({ value, label: t(`share.expire.${value}`) }))}
            value={clampShareExpire(storedExpire, supportedExpires)}
