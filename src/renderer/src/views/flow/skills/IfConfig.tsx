...
export const IfConfig: React.FC<SkillConfigProps> = ({ step, onChange, t }) => {
  ...
  return (
    <Stack gap="xs">
      <AppTextInput
        label={t('flow.skill.if.left')}
        placeholder={t('flow.skill.if.left.placeholder')}
      ...
      />
      <SelectDropdown
        label={t('flow.skill.if.operator')}
        options={IF_OPERATORS.map((op) => ({ value: op, label: t(`flow.skill.if.op.${op}`) }))}
        value={operator}
      ...
      {showRight && (
        <AppTextInput
          label={t('flow.skill.if.right')}
          placeholder={t('flow.skill.if.right.placeholder')}
          value={step.config.right ?? ''}
          onChange={(e) => onChange({ ...step.config, right: e.currentTarget.value })}
          size="sm"
        />
      )}
      <Text fz="xs" c="dimmed">{t('flow.skill.if.hint')}</Text>
    </Stack>
  );
};
