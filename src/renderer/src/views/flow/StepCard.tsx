...
export const StepCard: React.FC<StepCardProps> = ({
  ...
  const skillLabelKey = `flow.skill.${step.type}` as const;
      ...
      <Paper withBorder p="sm" radius="md" bg="var(--mantine-color-red-light)">
        <Stack gap="xs">
          <Group justify="space-between" wrap="nowrap" align="flex-start">
            ...
            <Tooltip label={t('flow.step.delete')} position="top">
              <ActionIcon variant="subtle" color="red" size="sm" onClick={() => removeStep(flowId, step.id)}>
                <Trash2 size={14} />
              </ActionIcon>
            </Tooltip>
          </Group>
          <Text fz="xs" c="dimmed">{t('flow.step.unknown.hint')}</Text>
        </Stack>
      </Paper>
    );
  }
      ...
      <Stack gap="xs">
        <Group justify="space-between" wrap="nowrap">
          ...
          <Group gap={6} wrap="nowrap">
            ...
            <Tooltip label={t('flow.step.moveUp')} position="top">
              <ActionIcon variant="subtle" size="sm" disabled={index === 0} onClick={() => moveStep(flowId, step.id, 'up')}>
                <ArrowUp size={14} />
              </ActionIcon>
            </Tooltip>
            <Tooltip label={t('flow.step.moveDown')} position="top">
        ...
        {!atomic && (
          <>
            ...
            <Box onFocusCapture={handleFocusCapture}>
              <ConfigEditor step={step} onChange={handleConfigChange} t={t} flowVariables={flowVariables} />
            </Box>
            <AvailableVarsHint
              prevSteps={prevSteps}
            ...
            {stepHasOutput(step) && (
              <AppTextInput
                label={t('flow.step.outputKey')}
                description={t('flow.step.outputKey.hint')}
                value={step.outputKey}
                onChange={(e) => updateStep(flowId, step.id, { outputKey: e.currentTarget.value })}
                size="xs"
                error={outputKeyError ? t(`flow.step.outputKey.error.${outputKeyError}`) : undefined}
              />
            )}
          </>
        )}
