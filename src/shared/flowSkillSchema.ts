...
export const DEFAULT_SKILL_CONFIG: Record<SkillType, Record<string, string>> = {
  ...
  rss: { url: '', cacheDays: '3', checkpoint: '', lastLinks: '' },
  stop: { value: '' },
  comment: { note: '' },
...
const EXAMPLE_FLOW_JSON = JSON.stringify(
  {
    ...
    steps: [
      { type: 'rss', label: 'Fetch feed', config: { url: 'https://www.engadget.com/rss.xml', cacheDays: '3' }, outputKey: 'rss_1' },
...
export function buildFlowGenerationPrompt(description: string): string {
  ...
  return [
    ...
    '- hotkey: {"type":"hotkey","keys":"CommandOrControl+Shift+Y"}  (an Electron accelerator: "+"-joined modifiers CommandOrControl/Alt/Shift/Super plus ONE key, or a bare media/volume/function key like "MediaPlayPause"; pick an uncommon combo to avoid collisions)',
