...
| 🔑   | **API キー不要**       | 有料 API ではなく各サービスの Web ページを使用——登録も支払いも一切不要。キーをお持ちなら、オプションの BYOK モードにも対応 |
...
![AgentFlow エディタ（RSS ステップ）](assets/screenshots/agentflow-editor-rss-step.png)
...
- 🧠 **AI に質問** — ChatGPT · Gemini · Perplexity · Duck.ai——または BYOK でお手持ちの API キーを利用
...
- **API キー持ち込み（BYOK）** — お手持ちの OpenAI 互換（OpenAI・OpenRouter・Together・Groq・ローカルサーバーなど）や Gemini API キーを追加のプロバイダーとして登録できます：種類を選び、ベース URL・モデル・キーを設定し、「**モデルを読み込む**」でエンドポイントが提供するモデルから選び、「**テスト**」で動作を確認します。BYOK インスタンスはチャットや AgentFlow で他のプロバイダーと同じように選べます。ブラウザモードが引き続きデフォルトで、キーは OS のキーチェーンで暗号化されます。
...
Yobi は、ChatGPT・Gemini・Perplexity・Duck.ai の **Web インターフェース** を自動化します。内蔵のブラウザウィンドウで、各サービスのサイトにプロンプトを入力し、ページから回答を読み取ります——あなたが手作業でやるのと同じことです。ログインが必要なのはそれを求めるサービスだけで、ログインはそのウィンドウ内で行います。デフォルトでは**公式 API もローカルモデルも使いません**。だからこそ、API キーも料金も不要なのです。（キーをお持ちの場合、オプションの BYOK モードでは任意の OpenAI 互換エンドポイントや Gemini API を直接呼び出せます——以下の説明はデフォルトのブラウザモードについてのものです。）
...
- **認証情報の暗号化** — Telegram トークン・SMTP パスワード・BYOK API キーは、OS のキーチェーン（Electron `safeStorage`）で暗号化されてからディスクに書き込まれます。
